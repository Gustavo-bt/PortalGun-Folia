...
public class PortalUtils {
    ...
    public void portalTeleport(Entity entity, Portal toPortal){
        ...
        Vector vec;
        double vecPower = Math.abs(entity.getVelocity().getX()+entity.getVelocity().getY()+entity.getVelocity().getZ()); if (vecPower < 0.3) vecPower = 0.3;
        ...
        vec = vec.multiply(2);
        final org.bukkit.util.Vector fVec = vec;
        entity.teleportAsync(loc).thenRun(() -> {
            if (!entity.isValid()) return;
            if (entity instanceof Player) {
                SchedulerUtil.runTaskLaterForPlayer(PortalGunMain.getInstance(), (Player) entity, () -> {
                    try {
                        if (entity.isValid()) {
                            entity.setVelocity(fVec);
