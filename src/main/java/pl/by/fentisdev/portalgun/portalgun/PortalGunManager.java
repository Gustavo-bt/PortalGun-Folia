...
public class PortalGunManager {
    ...
    private Object portalScheduler;
    @Getter
    ...
    public void startPortalSchedulerOld(){
    ...
    public void ensureRendered() {
        for (PortalGun portalGun : new ArrayList<>(getPortalGuns())) {
            if (!portalGun.isOnline()) continue;
            Portal p1 = portalGun.getPortal1();
            Portal p2 = portalGun.getPortal2();
            if (p1 != null && p1.getLoc1() != null && (p1.getUp() == null || p1.getDown() == null || p1.getUp().isDead() || p1.getDown().isDead())) {
                try { p1.renderPortal(); } catch (Throwable ignored) {}
            }
            if (p2 != null && p2.getLoc1() != null && (p2.getUp() == null || p2.getDown() == null || p2.getUp().isDead() || p2.getDown().isDead())) {
                try { p2.renderPortal(); } catch (Throwable ignored) {}
            }
        }
    }

    public void stopPortalScheduler(){
        SchedulerUtil.cancelTask(portalScheduler);
    }
        ...
        try {
            ...
            if (jsonObject != null) {
                ...
                for (Map.Entry<String, JsonElement> guns : jsonObject.getAsJsonObject("PortalGuns").entrySet()) {
                    ...
                    if (portal1.has("loc1")){
                        p1.loadPortalData(PortalUtils.getInstance().getLocationJson(portal1.getAsJsonObject("loc1")),
                    ...
                    if (portal2.has("loc1")){
                        p2.loadPortalData(PortalUtils.getInstance().getLocationJson(portal2.getAsJsonObject("loc1")),
    ...
    public void startPortalScheduler(){
        portalScheduler = SchedulerUtil.runRepeatingGlobal(PortalGunMain.getInstance(), () -> {
            if (Bukkit.getOnlinePlayers().isEmpty() || getPortalGuns().isEmpty()) return;
            ensureRendered();
            for (PortalGun portalGun : new ArrayList<>(getPortalGuns())) {
                if (!portalGun.isOnline()) continue;
                Location loc1 = portalGun.getPortal1().getLoc1();
                Location loc2 = portalGun.getPortal2().getLoc1();
                if (loc1 == null || loc2 == null) continue;
                if (!PortalConfig.getInstance().isInterdimensional() && loc1.getWorld() != loc2.getWorld()) continue;
                SchedulerUtil.runAtLocation(PortalGunMain.getInstance(), loc1, () -> {
                    if (!portalGun.isOnline() || portalGun.getPortal1().getLoc1() == null || portalGun.getPortal2().getLoc1() == null) return;
                    portalGun.verifyPortals();
                    for (Entity entity : portalGun.getPortal1().getEntityNearby()) {
                        EntityTeleportInPortalEvent event = new EntityTeleportInPortalEvent(portalGun, portalGun.getPortal1(), portalGun.getPortal2(), entity);
                        Bukkit.getPluginManager().callEvent(event);
                        if (!event.isCancelled()) PortalUtils.getInstance().portalTeleport(entity, portalGun.getPortal2());
                    }
                });
                SchedulerUtil.runAtLocation(PortalGunMain.getInstance(), loc2, () -> {
                    if (!portalGun.isOnline() || portalGun.getPortal1().getLoc1() == null || portalGun.getPortal2().getLoc1() == null) return;
                    for (Entity entity : portalGun.getPortal2().getEntityNearby()) {
                        EntityTeleportInPortalEvent event = new EntityTeleportInPortalEvent(portalGun, portalGun.getPortal2(), portalGun.getPortal1(), entity);
                        Bukkit.getPluginManager().callEvent(event);
                        if (!event.isCancelled()) PortalUtils.getInstance().portalTeleport(entity, portalGun.getPortal1());
                    }
                });
            }
            List<Player> remove = new ArrayList<>();
            for (Map.Entry<Player, Entity> h : new HashMap<>(getHolding()).entrySet()) {
                Player p = h.getKey();
                Entity held = h.getValue();
                if (p == null || !p.isOnline() || held == null || held.isDead()) { remove.add(p); continue; }
                SchedulerUtil.runTaskLaterForPlayer(PortalGunMain.getInstance(), p, () -> {
                    PortalGun pg = PortalUtils.getInstance().getPortalGun(p, p.getInventory().getItemInMainHand());
                    if (pg == null) pg = PortalUtils.getInstance().getPortalGun(p, p.getInventory().getItemInOffHand());
                    if (pg == null) return;
                    Location eyeLoc = p.getEyeLocation();
                    RayTraceResult t = p.getWorld().rayTraceBlocks(eyeLoc, eyeLoc.getDirection(), 3, FluidCollisionMode.NEVER, true);
                    Location nloc;
                    if (t != null) {
                        nloc = t.getHitPosition().toLocation(p.getWorld());
                        BlockFace bf = t.getHitBlockFace();
                        if (bf == BlockFace.NORTH || bf == BlockFace.SOUTH || bf == BlockFace.EAST || bf == BlockFace.WEST) {
                            Vector direction = p.getEyeLocation().getDirection().normalize();
                            double x = direction.getX() * (held.getBoundingBox().getWidthX() / 2);
                            double y = direction.getY() * (held.getBoundingBox().getHeight() / 2);
                            double z = direction.getZ() * (held.getBoundingBox().getWidthZ() / 2);
                            nloc.subtract(x, -y, z);
                        }
                        nloc.setYaw(p.getLocation().getYaw());
                        nloc.setPitch(p.getLocation().getPitch());
                        ...
                        nloc = p.getEyeLocation();
                        Vector direction = nloc.getDirection().normalize();
                        nloc.add(direction.getX() * a, direction.getY() * a - (held.getBoundingBox().getHeight() / 2), direction.getZ() * a);
                        nloc.setYaw(p.getLocation().getYaw());
                        nloc.setPitch(p.getLocation().getPitch());
                    }
                    held.teleportAsync(nloc);
                }, 1);
            }
            if (!remove.isEmpty()) {
                SchedulerUtil.runTaskLater(PortalGunMain.getInstance(), () -> { for (Player p : remove) removeHolding(p, null); }, 1);
            }
        }, 1, 1);
    }
}